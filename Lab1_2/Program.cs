...
namespace Lab1_2
{
    public class Program
    {
        ...
        static void OrderRowsByNumberOfSameValues(int[,] matrix)
        {
        ...
        static void SearchFirstNumberOfColWithNegValue(int[,] matrix)
        {
            ...
            for (int i = 0; i < matrix.GetLength(1); i++)
            {
                ...
                if (countNegValues == 0)
                {
                    Console.WriteLine($"Номер первого столбца, где нет отрицательных элементов {++i}");
                    marker = false;
                    break;
                }
                countNegValues = 0;
